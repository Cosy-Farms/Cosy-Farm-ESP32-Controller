...
void commandUpdate() {
    if (Serial.available() > 0) {
        ...
        if (cmd == 'W' || cmd == 'w') {
            ...
            prefs.clear();
            prefs.end();
            Serial.println("[SUCCESS] WiFi settings cleared. Restarting to use defaults from define.h...");
            delay(1000);
            ESP.restart();
        }
        ...
        if (cmd == 'C' || cmd == 'c') {
            ...
            Serial.println("Step 1: Enter New SSID:");
            while (Serial.available() == 0) vTaskDelay(pdMS_TO_TICKS(10));
            String newSsid = Serial.readStringUntil('\n');
            newSsid.trim();
            
            Serial.println("Step 2: Enter New Password:");
            while (Serial.available() == 0) vTaskDelay(pdMS_TO_TICKS(10));
            ...
            if (newSsid.length() > 0) {
                ...
                Serial.println("[SUCCESS] Credentials saved to NVS. Restarting...");
                delay(2000);
                ESP.restart();
            } else {
                Serial.println("[ERROR] SSID cannot be empty. Configuration aborted.");
            }
        }
        ...
        if (cmd == 'S' || cmd == 's') {
            ...
            prefs.end();
            Serial.println("[SUCCESS] Credentials updated in NVS. Restarting...");
            delay(1000);
            ESP.restart();
        }
        ...
        if (cmd == 'F' || cmd == 'f') {
            ...
            if (err == ESP_OK) {
                nvs_flash_init();
                Serial.println("[SUCCESS] All settings cleared. Restarting...");
                delay(1000);
                ESP.restart();
            } else {
